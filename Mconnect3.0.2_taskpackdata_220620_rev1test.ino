    ...
    if (!writeFile(SD, filenamesavesd , datasavesdcard)) {
      ...
      //For test RJK
       ...
       if(write_addeeprom == 0){
         int n =0;
          while(n<sizeof(xdataContinue)){
          Serial.println(xdataContinue[n]);
          for (int m = 0; m < addrsize; m++)
            {
              mac.writeAddress(write_addeeprom,xdataContinue[n][m]);
              write_addeeprom++;
            }
            Serial.println(write_addeeprom);
            n++;
...
void setup() {
  ...
  client.setCallback(IOTCallback);
   ...
   //task_packdata.start(taskPackdatcon,NULL,115200);
   task_packdata.start(taskPackdatcon,NULL,102400);
}
void loop() {
